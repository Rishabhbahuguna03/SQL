-- Practice questions:
-- Q.1. Find patients who are in services with above-average staff count.
...
FROM staff
GROUP BY service) AS service_counts));

-- Q.2. List staff who work in services that had any week with patient satisfaction below 70.
...
-- Q.3. Show patients from services where total admitted patients exceed 1000.
...
HAVING 
SUM(CASE
		WHEN patients_refused > 0 THEN 1
		ELSE 0
		END) >= 1
